...
function doPost(e) {
  try {
    ...
    switch(data.type) {
      ...
      case 'get_clients':    return repondre(getClients(ss, data));
      case 'get_licences':   return repondre(getLicences(ss, data));
...
// ── LECTURE CLIENTS pour admin ────────────────────────────────────────
function getClients(ss, data) {
  if (!jetonAdminValide(data.jeton, data.expire))
    return {ok: false, error: 'Non autorisé — reconnectez-vous'};
  const sheet = ss.getSheetByName('Clients');
  if (!sheet) return {ok: true, clients: []};
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    clients.push({
      ...
      prenom    : r[1] || '—',
      tel       : r[2] || '',
      codePromo : (r[3] && r[3] !== '—') ? r[3] : '',
      commercial: (r[4] && r[4] !== '—') ? r[4] : '',
      statut    : r[5] || 'Essai',
      duree     : r[6] || 1,
      debut     : r[7] || '',
      licencie  : r[5] === '✅ Licencié'
    });
  }
  return {ok: true, clients};
}

// ── LECTURE LICENCES pour admin ───────────────────────────────────────
function getLicences(ss, data) {
  if (!jetonAdminValide(data.jeton, data.expire))
    return {ok: false, error: 'Non autorisé — reconnectez-vous'};
  const sheet = ss.getSheetByName('Licences');
  if (!sheet) return {ok: true, licences: []};
  const rows = sheet.getDataRange().getValues();
  const licences = [];
  for (let i = 1; i < rows.length; i++) {
    const r = rows[i];
